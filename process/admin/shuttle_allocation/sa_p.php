...
if ($method == 'get_shuttle_allocation') {
	...
	sa.id AS sa_id, sa.out_5, sa.out_6, sa.out_7, sa.out_8, sa.day AS sa_day, sa.shift AS sa_shift, sa.shuttle_route AS sa_shuttle_route
		...
		LEFT JOIN 
		(SELECT id, emp_no, time_in, day, shift FROM t_time_in_out WHERE day = '$day' AND shift = '$shift') AS tio ON tio.emp_no = emp.emp_no
		LEFT JOIN 
		(SELECT id, emp_no, out_5, out_6, out_7, out_8, day, shift, shuttle_route FROM t_shuttle_allocation WHERE day = '$day' AND shift = '$shift') AS sa ON sa.emp_no = emp.emp_no
		WHERE emp.dept = '$dept'";
	if (!empty($section)) {
...
if ($method == 'set_out') {
	...
	foreach ($arr as $id) {
		$sql = "SELECT 
		tio.day, tio.shift, 
		emp.emp_no, emp.dept, emp.section, emp.line_no, emp.shuttle_route
		...
		$sql = "SELECT id FROM t_shuttle_allocation WHERE emp_no = '$emp_no' AND day = '$day' AND shift = '$shift' AND (out_5 != 0 OR out_6 != 0 OR out_7 != 0 OR out_8 != 0)";
		$stmt = $conn -> prepare($sql);
		$stmt -> execute();
		if ($stmt->rowCount() > 0) {
			foreach($stmt->fetchALL() as $j){
			...
			$sql = "INSERT INTO t_shuttle_allocation (`emp_no`, `dept`, `section`, `line_no`, `day`, `shift`, `shuttle_route`, `out_".$time."`, `set_by`) VALUES ('$emp_no', '$dept', '$section', '$line_no', '$day', '$shift', '$shuttle_route', 1, '".$_SESSION['full_name']."')";
			$stmt = $conn->prepare($sql);
			$stmt->execute();
		}
...
if ($method == 'get_shuttle_allocation_per_route') {
	...
	$sql = "SELECT shuttle_route, sum(out_5) as total_out_5, sum(out_6) as total_out_6, sum(out_7) as total_out_7, sum(out_8) as total_out_8 FROM t_shuttle_allocation WHERE day = '$day' AND shift = '$shift' AND dept = '$dept'";
	if (!empty($section)) {
		$sql = $sql . " AND section = '$section'";
	}
	if (!empty($line_no)) {
