...
function count_control_area_account_list($search_arr, $conn) {
	$query = "SELECT count(id) AS total FROM m_control_area_accounts WHERE";
	$params = [];
...
if ($method == 'count_control_area_account_list') {
	$emp_no = $_POST['emp_no'];
	$full_name = $_POST['full_name'];
	...
	echo count_control_area_account_list($search_arr, $conn);
}

if ($method == 'control_area_account_list_last_page') {
	$emp_no = $_POST['emp_no'];
	$full_name = $_POST['full_name'];
	...
	$number_of_result = intval(count_control_area_account_list($search_arr, $conn));
...
if ($method == 'control_area_account_list') {
	$emp_no = $_POST['emp_no'];
	$full_name = $_POST['full_name'];
	...
	$query = "SELECT id, emp_no, full_name, dept, section, line_no, shift_group, role, position, email FROM m_control_area_accounts WHERE";
	...
	if ($row) {
		do {
			...
			if (isset($_SESSION['emp_no_hr'])) {
				...
				echo '<td style="cursor:pointer;" class="modal-trigger" data-toggle="modal" data-target="#update_control_area_account" onclick="get_accounts_details(&quot;'.$row['id'].'~!~'.$row['emp_no'].'~!~'.$row['full_name'].'~!~'.$row['dept'].'~!~'.$row['section'].'~!~'.$row['line_no'].'~!~'.$row['role'].'~!~'.$row['shift_group'].'~!~'.$row['position'].'~!~'.$row['email'].'&quot;)">'.$c.'</td>';
			} else {
				echo '<tr style="cursor:pointer;" class="modal-trigger" data-toggle="modal" data-target="#update_control_area_account" onclick="get_accounts_details(&quot;'.$row['id'].'~!~'.$row['emp_no'].'~!~'.$row['full_name'].'~!~'.$row['dept'].'~!~'.$row['section'].'~!~'.$row['line_no'].'~!~'.$row['role'].'~!~'.$row['shift_group'].'~!~'.$row['position'].'~!~'.$row['email'].'&quot;)">';
...
if ($method == 'register_control_area_account') {
	$full_name = trim($_POST['full_name']);
	$emp_no = trim($_POST['emp_no']);
	$dept = trim($_POST['dept']);
	$section = trim($_POST['section']);
	...
	$check = "SELECT id FROM m_control_area_accounts WHERE emp_no = ?";
	$stmt = $conn->prepare($check);
	$params = array($emp_no);
	$stmt->execute($params);
	...
	if ($row) {
		...
		$query = "INSERT INTO m_control_area_accounts 
						(emp_no, full_name, dept, section, line_no, shift_group, role, position, email) 
					VALUES 
						(?, ?, ?, ?, ?, ?, ?, ?, ?)";
		
		$stmt = $conn->prepare($query);
		$params = array($emp_no, $full_name, $dept, $section, $line_no, $shift_group, $role, $position, $email);
...
if ($method == 'update_control_area_account') {
	$id = $_POST['id'];
	$emp_no = trim($_POST['emp_no']);
	$full_name = trim($_POST['full_name']);
	$dept = trim($_POST['dept']);
	...
	$query = "SELECT id FROM m_control_area_accounts 
				WHERE emp_no = ? AND 
					full_name = ? AND 
					dept = ? AND 
					section = ? AND 
	...
	if ($row) {
		...
		$query = "UPDATE m_control_area_accounts 
					SET shift_group = ?, role = ?, 
					position = ?, email = ? 
					WHERE id = ?";

		$stmt = $conn->prepare($query);
		$params = array($shift_group, $role, $position, $email, $id);
		...
		$stmt = $conn->prepare($query);
		$params = array($full_name, $dept, $section, $line_no, $shift_group, $role, $position, $email, $id);
...
if ($method == 'delete_control_area_account') {
	$id = $_POST['id'];
	$emp_no = '';

	$query = "SELECT emp_no FROM m_control_area_accounts WHERE id = ?";
	...
	if ($row) {
		...
		$query = "DELETE FROM m_control_area_accounts WHERE id = ?";
