...
// Set column headers 
$fields = array('#', 'Provider', 'ID No.', 'Name', 'Department', 'Section', 'Line No.', 'Shift Group', 'Shift', 'Time In 1', 'Time In 2', 'Time Out', 'IP', 'Status'); 
...
$sql = "SELECT 
	...
	tio.shift, FORMAT(tio.date_updated, 'HH:mm:ss') as time_in_1, FORMAT(tio.time_in, 'HH:mm:ss') as time_in_2, FORMAT(tio.time_out, 'HH:mm:ss') as time_out, tio.ip
	FROM m_employees emp
...
while($row = $stmt -> fetch(PDO::FETCH_ASSOC)) { 
	...
	if (!empty($row['time_in_1']) || !empty($row['time_in_2'])) {
		$row_status = 'Present';
	} else {
		$row_status = 'Absent';
	}

	$lineData = array($c, $row['provider'], $row['emp_no'], $row['full_name'], $row['dept'], $row_section, $row_line_no, $row['shift_group'], $row['shift'], $row['time_in_1'], $row['time_in_2'], $row['time_out'], $row['ip'], $row_status); 
