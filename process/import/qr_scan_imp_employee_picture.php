...
if (preg_match('/[^a-zA-Z0-9_-]/', $_POST['qr'])) {
...
if ($_FILES['photo']['error'] !== UPLOAD_ERR_OK) {
    exit("File upload failed. Error code: " . $_FILES['photo']['error']);
}
...
if (!in_array($imageType, [IMAGETYPE_PNG, 2])) {
...
if ($imageType === IMAGETYPE_PNG || $imageType == 2) {
    $foo = imagecreatefromjpeg($target_file);
} else {
...
if ($originalWidth > $newWidth) {
    ...
    imagepng($newImage, str_replace('jpg', 'png', $target_file));
...
// Build response
$response = [
    "message" => "{$_POST['qr']} received and processed",
    "json" => ''
];
